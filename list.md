...
31. Use DLSS(or similar technology) with "GeForce Now"(or other cloud gaming service) to upscale the game stream, it would lower the bandwidth requirements. Every game would have its own DLSS profile. The gaming service would stream the game in low resolution to the client, the client would load the correct DLSS profile and would use DLSS to upscale the game stream to a higher resolution. 
