...
The input of the neural network is the location/coordinates of the rectangle/window representing a part of the image. The input is four(4) floating point values X1,Y1,X2,Y2,  normalized in range from -1 to 1, they represent the location/coordinates of a rectangle/window(a part) inside the image. First we get the integer coordinates of the rectangle/window X1, Y1, X2, Y2 and normalize them so they are in range -1 to 1, by applying the following functions/formulas/formulae: for x values f(X) = (X/X_max * 2) -1 and for y values f(Y) = ((Y/Y_max) * 2) - 1 , where X_max represent the maximum value of the X axis of the image and the Y_max represent the maximum value of the Y axis of the image.
